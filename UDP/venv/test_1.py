...
def func1(str): ##识别
    ...
    while(index<length):
        str.replace(" ","")
        while (str[index] in operator or str[index] in delimiters):
            if(str[index]=="."and str[index-1].isdigit()):
                index+=1
                continue
            i = index

            if(index+1<=length):
                while(str[index+1] in operator or str[index+1] in delimiters):
                    sign.append(index)
                    index += 1

            sign.append(index)
            if(i!=index):
                list.append("(s,\"" + str[i:index+1] + "\")")
            else:
                list.append("(s,\"" + str[i] + "\")")
            index += 1
        index += 1
...
if __name__ == '__main__':
    #str = input("请输入一个字符串:")
    str = "if (r >= 2.73246) C = 2*213*pi"  ##测试
    func1(str)